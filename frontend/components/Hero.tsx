...
import { ArrowRight, Camera, Mic, ScanSearch, Sparkles, type LucideIcon } from "lucide-react";
...
const capabilityCards: CapabilityCard[] = [
  { id: "natural-voice", icon: Mic, title: "Natural voice", copy: "Interruptible, fast, conversational" },
  { id: "camera-first", icon: Camera, title: "Camera-first", copy: "The live scene is always the hero" },
  { id: "freeze-explain", icon: ScanSearch, title: "Freeze + explain", copy: "Precise grounded visual analysis" }
];
        ...
        <div className="mt-10 grid gap-3 sm:grid-cols-3">
          {capabilityCards.map((card) => (
            <div key={card.id} className="glass rounded-2xl p-4">
              <card.icon className="h-4 w-4 text-mint" />
              <p className="mt-3 text-sm font-medium">{card.title}</p>
              <p className="mt-1 text-xs text-white/55">{card.copy}</p>
            </div>
          ))}
        </div>
      </motion.div>
