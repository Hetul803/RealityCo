...
export function AgentStatusPill({ state, customLabel }: { state: SessionState; customLabel?: string }) {
  const config = stateConfig[state];

  return (
    <span className="relative inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/35 px-4 py-2 text-xs text-white/90 transition-all duration-300">
      ...
      <span className="tracking-wide">{customLabel ?? config.label}</span>
    </span>
  );
}
