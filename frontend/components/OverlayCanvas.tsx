    ...
    <div className="pointer-events-none absolute inset-0">
      {annotations.map((a, idx) => {
        ...
        return (
          ...
          >
            <span
              className={`absolute left-0 max-w-[160px] truncate rounded-md border border-white/20 bg-black/80 px-2 py-1 text-[10px] font-medium tracking-wide text-white ${
                labelBelow ? "top-0 translate-y-1" : "-top-1 -translate-y-full"
              }`}
              style={{ transform: `${labelBelow ? "translateY(4px)" : "translateY(-100%)"} translateX(${stackOffset}px)` }}
