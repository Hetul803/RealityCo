...
@router.post("/vision/analyze", response_model=Union[VisionAnalysisResponse, InterviewCoachResponse])
async def analyze_frame(
    req: FrameAnalyzeRequest,
    gemini: GeminiService = Depends(get_gemini_service),
    storage: SnapshotStorageService = Depends(get_storage_service),
) -> Union[VisionAnalysisResponse, InterviewCoachResponse]:
    try:
    ...
    if req.mode == AnalysisMode.interview_coach:
        response = gemini.analyze_interview(
        ...
        session_manager.add_turn(req.session_id, "assistant", response.spoken_feedback)
    else:
        if not req.image_base64:
            raise HTTPException(status_code=400, detail="image_base64 is required in visual_guide mode")
